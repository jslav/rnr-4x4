    ...
    switch (pixelformat) {
           ...
           /*interleaved 422*/
            ...
            case V4L2_PIX_FMT_ABGR32:
                    ...
                    break;
            /*non-interleaved 420*/
            //case V4L2_PIX_FMT_YUV420P:
            //case V4L2_PIX_FMT_YVU420P:
            //case V4L2_PIX_FMT_YUV420P2:
            case V4L2_PIX_FMT_NV12:
            //case V4L2_PIX_FMT_TILED_NV12:
                    bpp = 12;
                    break;
            default:
                    bpp = 8;
...
int main(const int argc, const char **argv) {
    ...
    void *buffer_stdin = NULL;
    int buffer_stdin_size = 0;
	...
	while ((opt = getopt(argc, argv, "v:i:p:o:w:h:f:m:")) != -1) {
        switch (opt) {
            case 'v':
    ...
    if (strlen(input_file) > 0) {
        ...
        if (strlen(output_file) <= 0) {	
            data_from_stdin = 1;
            buffer_stdin_size = width * height * fmt_to_bpp(cap_dev_pix_fmt)/8;
            buffer_stdin = malloc(buffer_stdin_size);
    ...
    if (data_from_stdin == 0) {
	   printf("Runnig h264 encoding from V4L device %s...\n", VIDEO_DEV);
        ...
        printf("Runnig h264 encoding %s from stdin or pipe/fifo...\n", str_fmt);
    }
	// rmmod
    ...
    while (1) {
        ...
        if (data_from_stdin == 0) {
            ...
            pdata = buffers[buf.index].start;
            plen = buf.bytesused;
        } else {
            int rd;

            rd = read_frame(in, buffer_stdin, buffer_stdin_size);
            if (rd == 0) {
                break;
            }
            pdata = buffer_stdin;
            plen = buffer_stdin_size;
        }
        ...
        for (i = 0;i < N_LB_DEV;i++) {
        	...
        	if (th_start[i].lb_codec == H264_LB) {  
...
#if defined(CPU_HAS_NEON) 		        	
        			    UYVYToNV12_neon(pdata, src_stride,
		               		   input_buf, dst_stride_y,
		               		   input_buf + uv_offset, dst_stride_uv,
		                       width, height);
#else
        			    uyvy422toNV12(width, height, pdata, input_buf);
#endif        			
		    	     } else if (cap_dev_pix_fmt == V4L2_PIX_FMT_YUYV) {
#if defined(CPU_HAS_NEON) 
		    		    YUYVToNV12_neon(pdata, src_stride,
		               		   input_buf, dst_stride_y,
		               		   input_buf + uv_offset, dst_stride_uv,
		                       width, height);
#else		    		
		    		    yuyv422toNV12(width, height, pdata, input_buf);
#endif
		    	    } else if (cap_dev_pix_fmt == V4L2_PIX_FMT_NV12) {
                        // copy without converting if NV12
                        memcpy(input_buf, pdata, plen);
                    } else {
		    		    continue;
		    	    }
        		...
        		pb = obtain_lbck_current_input_buf(th_start[i].lb_fd, th_start[i].lb_nbuf, th_start[i].lb_pbuf, &dev_ibuf);
        		...
        		if (th_start[i].pix_format == cap_dev_pix_fmt) {
                    ...
                    if (cap_dev_pix_fmt == V4L2_PIX_FMT_UYVY) {
            			UYVYTo420P_neon(pdata, src_stride,
    		               		   pb, dst_stride_y,
    		               		   pb + u_offset, dst_stride_uv,
    		               		   pb + v_offset, dst_stride_uv,
    		                       width, height);
                    } else if (cap_dev_pix_fmt == V4L2_PIX_FMT_YUYV) {
                        YUYVTo420P_neon(pdata, src_stride,
                                   pb, dst_stride_y,
                                   pb + u_offset, dst_stride_uv,
                                   pb + v_offset, dst_stride_uv,
                                   width, height);
...
#else 
        		uyvy422to420(width, height, pdata, pb);
#endif
				len = size_out;				
    ...
    if (data_from_stdin && buffer_stdin) {
        free(buffer_stdin);
    }
#endif	
