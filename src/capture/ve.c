...
#ifndef __aarch64__	
	printf("REGS pa: %08X\n", info.registers);
	printf("MEMORY pa %08X, page offset %08X size %d\n", info.reserved_mem, ve.first_memchunk.mem.phys, ve.first_memchunk.mem.size);
#else 
	printf("REGS pa: %016lX, va: %016lX\n", info.registers, (unsigned long)ve.regs);
	printf("MEMORY pa %016lX, page offset %08X size %d\n", info.reserved_mem, ve.first_memchunk.mem.phys, ve.first_memchunk.mem.size);
#endif	
	return 1;
...
struct ve_mem *ve_malloc(int size) {
	...
	if (addr == MAP_FAILED) {
		ret = NULL;
		printf("VE malloc: mmap failed with errno %d\n", errno);
		goto out;
	}
...
void ve_flush_cache(struct ve_mem *mem) {
...
#endif		
	{
		struct cedarv_cache_range range =
		{
			.start = (unsigned long)mem->virt,
			.end = (unsigned long)mem->virt + mem->size
		};
